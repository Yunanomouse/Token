...
| `examples/cantax.py` | Shared calculation library — brackets, BPA phase-outs, Ontario surtax & health premium, payroll — read straight from the JSON |
...
| `examples/personal_tax_outline_2026.xlsx` | Live-formula Excel outline for an employee with side trading income, built by `scripts/build_tax_workbook.py` |
| `examples/openbb_quickstart.py` | Fetches a year of daily prices for a list of tickers and saves them as CSVs |
| `examples/openrouter_quickstart.py` | Sends a prompt (optionally with one of these datasets attached) to any model on OpenRouter |
| `tests/` | `unittest` suite covering the dataset's internal consistency and the example scripts |
...
# Compare sole-proprietor, RRSP and CCPC outcomes on $150,000 of trading profit
...
# Check the dataset and the examples still agree
python3 -m unittest discover -s tests
```

All JSON files are plain UTF-8 with no dependencies; rates are decimals (0.14 = 14%), amounts are CAD, and bracket `up_to: null` means no upper bound. Two shapes are worth knowing: a jurisdiction's `basic_personal_amount` is a number, `null` where no 2026 figure has been published (PEI), or a `{max, min}` pair where the amount is phased out by income (federal and Yukon); and Ontario's `health_premium.bands` give `min(cap, base + rate x (income - income_over))` for the band a given income falls in.

The dataset itself needs nothing but Python 3.9+. Only two extras do: `examples/openbb_quickstart.py` needs `pip install openbb`, and `scripts/build_tax_workbook.py` needs `pip install openpyxl`. `examples/openrouter_quickstart.py` needs an `OPENROUTER_API_KEY` but no packages.
...
The suite needs no third-party packages and makes no network calls. It checks the dataset's internal arithmetic (contribution maximums against their rates and ceilings, sales-tax totals against their components, TFSA cumulative room against the annual limits, brackets ascending and open-ended), that `data/csv/` is not stale relative to `data/json/`, that the calculations match the statutory tables independently written out in the tests, and that both example scripts run for every jurisdiction and fail with a readable message rather than a traceback on bad input.
